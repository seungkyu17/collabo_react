...
//useParams 혹은 url에 들어있는 동적 파라미터의 값을 챙길때 사용합니다.
import { useParams } from "react-router-dom";

/* 상품 수정 페이지입니다.
    [상품 수정과 다른 점]
     기본 키인 상품의 'id 정보'가 넘어옵니다.
     id를 사용하여 기존에 입력했던 상품에 대한 정보를 미리 보여주어야 합니다.(useEffect 훅 사용)

step 01 :
 기존 수정 폼 양식을 복사합니다.(폼 양식 생성 : 이미지 - 물건)
    [기존 상품 정보 읽기]
     'get 방식'을 사용하여 해당 상품의 정보를 읽어옵니다.
    [테스트 시나리오]
     특정 상품에 대하여 '수정' 버튼을 클릭하면, 이전 상품 정보들이 입력란에 보여야 합니다.
    [상품 수정과 동일한 항목]
     ControlChange 함수, FileSelect 함수
...
function App() {
    ...
    //product는 백엔드에게 넘겨줄 상품 수정 정보를 담고 있는 객체
    ...
    }, [id]); //id 값이 변경될때마다 화면을 're-rendering' 시켜야 합니다.
    ...
    const FileSelect = (event) => {
        ...
        //FileReader 는 웹 브라우저에서 제공해주는 내장 객체로, 파일 읽기에 사용 가능합니다.
    ...
    const SubmitAction = async (event) => {
        ...
        try {
            ...
            //put() 메소드는 리소스를 "수정" 하고자할때 사용하는 메소드입니다. -- '거의 만든 사람의 의미 부여로 보인다.'
            ...
            //수정이 이루어지고 난 후 상품 목록 페이지로 이동합니다.
