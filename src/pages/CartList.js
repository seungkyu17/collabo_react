...
function App({ user }) {
    ...
    const toggleCheckBox = (cartProductId) => {
        ...
        setCartProducts((previous) => {
            //'!product.checked' 는 체크 상태를 'toggle' 시키는 역할을 합니다.
    ...
    const changeQuantity = async (cartProductId, quantity) => {
        ...
        try {
            //사용 예시 : 100번 항목을 10개로 수정 해주세요.
            //'http://localhost:9000/cart/edit/100?quantity=10'
            const url = `${API_BASE_URL}/cart/edit/${cartProductId}?quantity=${quantity}`;

            //'patch 동작'은 전체가 아닌 일부 데이터를 변경하고자 할때 사용됩니다.
            //'Spring Boot' 의 'WebConfig 클래스' 안의 'addCorsMappings() 메소드' 를 참조하시길 바랍니다.
            const response = await axios.patch(url);

            console.log(response.data || '');

            //'cartProducts'의 '수량 정보'를 갱신합니다.
            setCartProducts((previous) => {
                const updatedProducts = previous.map((product) =>
                    product.cartProductId === cartProductId
                        ...
                        cartProducts.map((product) => (
                            <tr key={product.cartProductId}>
                                <td className="text-center align-middle">
                                    {/* 어떤 체크인지 식별하기 위해 'toggleCheckBox 함수' 에 'cartProductId' 를 넘기도록 합니다. */}
                                    <Form.Check
                                        type="checkbox"
